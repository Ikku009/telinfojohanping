...
_k = base64.b64decode("cm9vdHhzdXJ5YW5zaA==").decode()
_b = base64.b64decode("aHR0cHM6Ly90Zy10by1udW0tc2l4LnZlcmNlbC5hcHAv").decode()
...
def buildResultMsg(data):
    ...
    lim   = safeVal(u.get("your_limit"))
    used  = safeVal(u.get("your_usage_today"))
    rem   = safeVal(u.get("your_remaining_today"))
    rtime = safeVal(data.get("response_time"))
    ...
    msg = (
        ...
        f"<b>API QUOTA</b>\n"
        f"<code>{'━'*30}</code>\n"
        f"<b>Limit</b>  <code>{lim}</code>   <b>Used</b>  <code>{used}</code>   <b>Left</b>  <code>{rem}</code>\n"
        f"\n"
        f"<i>Response time: {rtime}  —  @Joh4n404</i>"
    )
    return msg
...
async def cmdAdmin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    msg = (
        f"<b>ADMIN ACCESS</b>\n\n"
        f"Enter the admin password to continue."
    )
...
async def cbAdminLookups(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for l in chunk:
        ...
        lines.append(
            ...
            f"Status: <code>{'Success' if l.get('success') else 'Failed'}</code>   "
            f"Time: <code>{ts}</code>"
        )
...
async def cbTgToNum(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "<i>Example:  Joh4n404  or  @Joh4n404</i>",
        parse_mode=ParseMode.HTML,
...
async def receiveInput(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if err or data is None:
        await msg.reply_text(
            ...
            "<i>This is not an issue with your input. Try again shortly.</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=afterResultKb()
        )
        logLookup(u.id, u.username, u.first_name, query, None, False)
